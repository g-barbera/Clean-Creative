    ...
    private static EntryCollector modifier9(EntryCollector entryCollector) {
        return (displayContext, entries) -> {
            ...
            displayContext.lookup().getOptionalWrapper(RegistryKeys.ENCHANTMENT).ifPresent(registryWrapper -> {
                ...
                registryWrapper.streamEntries().map(RegistryEntry::value)
                    .filter(enchantment -> set.contains(enchantment.target))
                    .flatMap(enchantment -> IntStream.rangeClosed(enchantment.getMinLevel(), enchantment.getMaxLevel()).mapToObj(level -> EnchantedBookItem.forEnchantment(new EnchantmentLevelEntry(enchantment, level))))
